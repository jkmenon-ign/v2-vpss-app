...
int XV_HcrsmplInitialize(XV_Hcresampler_l2 *InstancePtr, u16 DeviceId)
{
  ...
  Status = XV_hcresampler_Initialize(&InstancePtr->Hcr, (const char *)DeviceId);
...
void XV_HCrsmplLoadDefaultCoeff(XV_Hcresampler_l2 *InstancePtr)
{
  ...
  printf("Number of Taps selected: %d \n", numTaps);
  switch(numTaps)
  {
		                                   ...
		                                   XV_HCRESAMPLER_CONVERSION convType)
{
  ...
  for(phase = 0; phase < XV_HCRSMPLR_MAX_PHASES; ++phase)
  {
    ...
    {
          printf("H-reampler tap coefficient: %d \n", pHCrsmplr->coeff[convType][phase][offset+tap]);
	  XV_hcresampler_WriteReg(pHCrsmplr->Hcr.Config.BaseAddress, XV_HCRESAMPLER_CTRL_ADDR_HWREG_COEFS_0_0_DATA
                      + (regcount * 8),pHCrsmplr->coeff[convType][phase][offset+tap]);
	  ++regcount;   
...
void XV_HCrsmplDbgReportStatus(XV_Hcresampler_l2 *InstancePtr)
{
  ...
  {
    printf("Resampling Type:  %s\r\n", RsmplrTypeStr[pHCrsmplr->Config.ResamplingType]);
  }
  else
  {
    printf("Resampling Type:  Unknown\r\n");
  }
    printf("Video Format In:  %s\r\n",
		   XVidC_GetColorFormatStr((XVidC_ColorFormat)vidfmtIn));
    printf("Video Format Out: %s\r\n",
  ...
  if(pHCrsmplr->Config.ResamplingType == XV_HCRSMPLR_TYPE_FIR)
  {
	...
	coeff = XV_vcresampler_ReadReg(pHCrsmplr->Config.BaseAddress,
			(XV_HCRESAMPLER_CTRL_ADDR_HWREG_COEFS_0_0_DATA + (regcount*8)));
        printf("%4d ",coeff);
	++regcount;
      }
    }
