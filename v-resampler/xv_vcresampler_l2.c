...
int XV_VcrsmplInitialize(XV_Vcresampler_l2 *InstancePtr, u16 DeviceId)
{
  ...
  if(InstancePtr != NULL)
    printf("InstancePtr not NULL \n");

  /* Setup the instance */
  memset(InstancePtr, 0, sizeof(XV_Vcresampler_l2));
 
  Status = XV_vcresampler_Initialize(&InstancePtr->Vcr, (const char *)DeviceId);
		                                   ...
		                                   XV_VCRESAMPLER_CONVERSION convType)
{
  ...
  for(phase = 0; phase < XV_VCRSMPLR_MAX_PHASES; ++phase)
  {
	...
	XV_vcresampler_WriteReg(pVCrsmplr->Vcr.Config.BaseAddress, 
		(XV_VCRESAMPLER_CTRL_ADDR_HWREG_COEFS_0_0_DATA + (regcount*8)),
			pVCrsmplr->coeff[convType][phase][offset+tap]); 
	  ++regcount;
    }
  }
}
    ...
    for(phase=0; phase < XV_VCRSMPLR_MAX_PHASES; ++phase)
    {
      ...
      for(tap=0; tap < numTaps; ++tap)
      {
	   // coeff = Xil_In32((baseAddr+(regcount*8)));
	    coeff = XV_vcresampler_ReadReg(pVCrsmplr->Config.BaseAddress,
			(XV_VCRESAMPLER_CTRL_ADDR_HWREG_COEFS_0_0_DATA + (regcount*8)));
            printf("%4d ",coeff);
	    ++regcount;
      }
